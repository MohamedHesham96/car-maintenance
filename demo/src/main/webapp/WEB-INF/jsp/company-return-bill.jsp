			...
			<div class="shadow ">
				...
				<div class="card border-warning text-warning  "
					...
					<div class="card-body font-weight-bold ">
						...
						<input disabled="disabled"
							value="المورد : ${companyBillReturn.company.name}"
							class="form-control text-center btn-outline-warning  mb-2 font-weight-bold" />


						<form:form method="get" action="add-item-to-company-return-bill"
							modelAttribute="item">
			...
			<div class="mr-4 col-8 shadow pb-3">
							...
							<c:forEach var="itemTemp"
								...
								<tr>
									...
									<td class="border-primary"><a
										href="delete-companyReturnBillItem?companyReturnBillItemId=${itemTemp.id}"
				...
				<div class="float-left pt-sm-4">
					...
					<form:form>
						...
						<a
							href="delete-companyReturnBill?companyReturnBillId=${companyBillReturn.id}"
						...
						<c:if
							...
							<a
								href="save-companyReturnBill?companyReturnBillId=${companyBillReturn.id}"
								onclick="return confirm('هل انت متأكد من حفظ الفاتورة ؟')"
								class="btn btn-success mr-1 shadow font-weight-bold ${companyBillReturnItems.size() eq 0 ? 'disabled' : ''} ">
								حفظ</a>

							<a
								href="show-printView?companyReturnBillId=${companyBillReturn.id}"
								onclick="return confirm('هل انت متأكد من طباعة الفاتورة ؟')"
								class="btn btn-primary mr-1 shadow font-weight-bold ${companyBillReturnItems.size() eq 0 ? 'disabled' : ''} ">
								طباعة</a>
